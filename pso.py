...
class PSO:
    ...
    def launch(self):
        ...
        if rank == 0:
            ...
            # if restart file is provided, load the swarm state and prepare log
            # file
            if self.params.restart_load!="NA":
                print ">> recovering swarm state from file %s..." \
                        % self.params.restart_load
                if os.path.isfile(self.params.restart_load)==1 :
                    swarm.load_restart(self.params.restart_load)
                else:
                    print "ERROR: restart file %s not found!" \
                            % self.params.restart_load
                    sys.exit(1)

                # if old logfile is available backup it and create a new one
                # with the same name
                # containing the lines generated until the restart file time
                if os.path.isfile(self.params.output_file)==1:

                    shutil.copy2(self.params.output_file,
                            "%s.bkp"%self.params.output_file)

                    fout_log = open(self.params.output_file, "w")
                    fin_log = open("%s.bkp"%self.params.output_file,"r")
                    totlines = self.params.n_particles * \
                            (swarm.repeat+1) * (swarm.ts+1)
                    for i in xrange(0,totlines,1):
                        line = fin_log.readline()
                        fout_log.write(line)

                    fin_log.close()
                    print ">> logging data in file %s, previous log backed up" \
                            + "as %s.bkp" \
                            % (self.params.output_file, self.params.output_file)

                #if logfile is not found, create a new one
                else:
                    print "WARNING: previous log file %s not found," \
                            + "starting a new one..."
                    fout_log = open(self.params.output_file, "w")

                print ">> optimization restored from repetition %s and" \
                        + "step %s" % (swarm.repeat+1,swarm.ts+1)

            #generate a random swarm otherwise
            else:
                # indent in real, as part of else just above.
        ...
        # (get current timestep and repeat from swarm state)
        comm.Barrier()
        ...
        # prepare repellers scaling factor, Xx the repeller should push
        # according to the size of the cel right?
        if self.params.repel:
            self.params.scale_repeller = self.space.cell_size \
                                        * self.params.repel_factor
        ...
        for r in xrange(start_repeat, self.params.repeat, 1):
            ...
            for ts in xrange(start_ts,self.params.max_steps,1):
                ...
                # every node elaborates the data. From received multidimensional
                # array, everybody extract the array corresponding to its rank,
                ...
                while p < self.params.n_particles :
                    ...
                    # getting position, velocity, max position (of this
                    # particle), max value, fitness, repellers
                    [part_pp, part_pv, part_pmp, \
                            part_pmv, part_cv, part_repel] = part.run()
                ...
                # data updated by all CPU is sent back to root (root data
                ...
                # root will reshape the received lists in new ordered
                # multidimensional arrays
                if rank == 0:
                    max_lines = int(
                        np.ceil(
                            float(self.params.n_particles) / float(size)
                        )
                    )
                    ...
                    if self.params.repel:
                        ...
                        cnt_rep = 0
                        for j in xrange(0, len(r_collect), 1):
                            r_tmp = r_collect[j]
                            #iterate over list of repellers per processor
                            for i in xrange(0, len(r_tmp), 1):
                                cnt_rep += 1
                                #print ">>> now repelling from %s"%r_tmp[i]
                                swarm.repellers.append(r_tmp[i])

                        if cnt_rep>0:
                            print ">>> added %s new repellers " \
                                    + "(tot. repellers: %s)" \
                                    % (cnt_rep,len(swarm.repellers))
                    ...
                    # if needed, write a swarm restart file (backup previous
                    # one, in case the computer crashes when writing the
                    # restart)
                    if (self.params.restart_freq > 0 and
                            np.mod(ts+1, self.params.restart_freq)) == 0:
                        print ">> saving swarm state at repetition %s," \
                                + "step %s..." % (r+1,ts+1)
                        ...
                        if os.path.isfile(self.params.restart_save)==1:
                            shutil.copy2(
                                self.params.restart_save,
                                "%s.old" % self.params.restart_save
                            )
...
class Swarm:
    ...
    def __init__(self):
        # needed to build a proper restart (save ts and repeat at last restart
        # time)
        self.ts = 0
        self.repeat = 0
        self.repellers = []
...
class particle :
    ...
    def run(self) :
        ...
        # find best neighbor and save in self.best_pos the position of its best
        # found fitness
        if self.params.neigh_type == "indexed":
            ...
            if self.n == 0:
                ...
                self.roi_val = np.array([
                    self.swarm.part_max_val[self.params.n_particles-1],
                    self.swarm.part_max_val[0],
                    self.swarm.part_max_val[1]
                ])
                self.roi_pos = np.array([
                    self.swarm.part_max_pos[self.params.n_particles-1],
                    self.swarm.part_max_pos[0],
                    self.swarm.part_max_pos[1]
                ])
            elif self.n == self.params.n_particles-1:

                self.roi_val = np.array([
                    self.swarm.part_max_val[self.n-1],
                    self.swarm.part_max_val[self.n],
                    self.swarm.part_max_val[0]
                ])
                self.roi_pos = np.array([
                    self.swarm.part_max_pos[self.n-1],
        ...
        self.local_max = self.params.cn \
                        * np.random.rand(len(self.space.low)) \
                        *(self.local)
        ...
        self.test = np.logical_and(
            abs(self.glob) > abs(
                self.best_pos + self.space.cell_size - self.row_pos
            ),
            self.space.boundary_type == 0
        )
        self.glob[self.test] = self.best_pos[self.test] \
                + self.space.cell_size[self.test] - self.row_pos[self.test]
        ...
        self.global_max = self.params.cp * np.random.rand(len(self.space.low)) \
                            * (self.glob)
        ...
        self.row_vel_new_tmp[self.test] = np.multiply(
                np.sign(self.row_vel_new_tmp[self.test]),
                self.space.cell_size[self.test]
        )

        ### flag, kick, reseed ###
        # update particle position according to velocity if speed is high
        # enough (wrapping on boundary conditions)
        self.velocity = float(np.sqrt(np.dot(
        ...
        if self.velocity > self.params.kar :
            ...
            [self.row_pos_new, self.row_vel_new] = self.space.check_boundaries(
                    self.row_pos_new_tmp,
                    self.row_vel_new_tmp
            )
        ...
        # if velocity is too low, launch the kick and reseed procedure
            ...
            # random kick particles being almost motionless and in a minima 
            # with bad fitness
            if self.swarm.current_val[self.n] > self.params.accept :
                #print ">>> kicking particle %s having fitness %s and " \
                #   + "velocity %s" \
                #   % (self.n,self.swarm.current_val[self.n],self.velocity)
                print ">>> kicking particle %s (fitness = %s)" \
                        % (self.n,self.swarm.current_val[self.n])
                self.row_vel_new_tmp = rand_set(
            ...
            # random kick and reseed particles being almost motionless and 
            # having a good fitness (needless to check)
            else :
                #print ">>> reseeding particle %s having fitness %s and " \
                #        + "velocity %s" \
                #        % (self.n,self.swarm.current_val[self.n],self.velocity)
                print ">>> reseeding particle %s (fitness = %s)" \
                        % (self.n, self.swarm.current_val[self.n])

                self.row_vel_new = rand_set(
                        -self.space.cell_size,
                        self.space.cell_size
                )
        ...
        if self.params.repel :
            ...
            for n in xrange(0,len(self.swarm.repellers),1):
                # if the particle is close to a repulsion point in a certain 
                # dimension, add its biasing contribution
                self.diff = self.row_pos-self.swarm.repellers[n]
                test = self.diff < self.space.cell_size \
                                    * self.params.repel_factor
                if np.any(test):
                    #print "repelling now..."
                    self.bias[test] -= self.space.cell_size[test] \
                            * self.params.repel_factor \
                            / (self.diff[test]**2)
                    #print "done!"

            # bias velocity and position with repeller, and verify boundary 
            # conditions
            self.test = np.abs(self.bias) > self.space.cell_size
            self.bias_tmp = np.multiply(np.sign(
                self.bias[self.test]),
                self.space.cell_size[self.test]
            )
        ...
        # check whether a new max has been found and store all the max and 
        # max_pos in temporary buffers
        ...
        if self.flag:
            return [self.row_pos_new, self.row_vel_new, self.max_pos_new, \
                    self.max_val_new, self.f, self.row_pos]
        else:
            return [self.row_pos_new, self.row_vel_new, self.max_pos_new, \
                    self.max_val_new, self.f, np.array([])]



def rand_set(min_a, max_a):
    out = np.zeros(len(min_a))
    for i in xrange(0, len(min_a), 1):
        out[i] = min_a[i] + np.random.rand(1)[0] * (max_a[i] - min_a[i])
    return out

def reformat(product, max_lines, size):
    m = []
    for i in xrange(0, max_lines, 1):
